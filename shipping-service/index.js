      ...
      eachMessage: async ({ topic, partition, message }) => {
        ...
        try {
          ...
          if (SERVICE === 'shipping-service' && topic === 'order.completed') {
            ...
            setTimeout(async () => {
              await producer.send({ topic: 'shipment.sent', messages: [{ key: orderId || uuidv4(), value: JSON.stringify({ eventType: 'SHIPMENT_SENT', data: { orderId } }) }] });
              console.log('shipping-service: published shipment.sent for', orderId);
            }, 500);
          }
        } catch (err) { console.error(`${SERVICE}: error handling message`, err); }
      }
    });
  } catch (err) {
...
const publish = async (topic, key, obj) => {
  try { await producer.send({ topic, messages: [{ key, value: JSON.stringify(obj) }] }); console.log(`${SERVICE}: published ${topic} - ${key}`); }
  catch (err) { console.error(`${SERVICE}: publish error`, err); }
};
